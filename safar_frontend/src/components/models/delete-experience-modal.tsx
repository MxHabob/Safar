...
import { useDeleteExperienceMutation } from "@/redux/services/api"
...
export default function DeleteExperienceModal() {
  ...
  const onDelete = async () => {
    ...
    try {
      await toastPromise(deleteExperience(experience.id).unwrap(), {
        loading: "Deleting experience...",
        success: "Experience deleted successfully!",
        error: (error) => `Failed to delete experience: ${error.data?.message || "Unknown error"}`,
      })
      onClose()
    } catch (error) {
