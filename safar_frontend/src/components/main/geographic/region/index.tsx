...
import { useGetRegionQuery } from "@/core/services/api";


export const RegionsPageContent = ({id}:{id:string}) => {
    const {data ,error,isLoading } = useGetRegionQuery(id)

  if (isLoading) {
    return (
      <RegionsPageContent.Skeleton/>
    )
  }
  if (error) {
    return (
    ...
    <div className="flex min-h-screen flex-col">
      <div className="container mx-auto px-4 py-2 flex justify-between text-xs">
        <BreadcrumbWithDropdown
            items={[{ label: "Home", href: "/" }, { label: `${data?.country?.name || "countries"}`, href: `/countries/${data?.country?.id || ""}` },{ label: `${data?.country?.name || "city"}`, href: `/cites/${data?.country?.id || ""}` }, { label: `${data?.name}` }]}
          />
        </div>
...
RegionsPageContent.Skeleton = function CountriesPageContentSkeleton() {
  return (
    <div className="flex min-h-screen flex-col">
      <div className="container mx-auto px-4 py-2 flex justify-between text-xs">
