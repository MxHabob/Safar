...
import { useCancelBookingMutation, useGetBookingsQuery, useGetUpcomingBookingsQuery } from "@/core/services/api"
...
export default function BookingsPageContent() {
  ...
  const { data: apiBookings } = useGetBookingsQuery({})
  const { data: apiUpcomingBookings } = useGetUpcomingBookingsQuery({})
  const { bookings: realtimeBookings, upcomingBookings: realtimeUpcomingBookings } = useRealtimeBookings()
  const allBookings = [...(apiBookings?.results || []), ...realtimeBookings]
  const upcomingBookings = [...(apiUpcomingBookings?.results || []), ...realtimeUpcomingBookings]
  const [cancelBooking] = useCancelBookingMutation()
  const bookings = showUpcoming ? upcomingBookings : allBookings
