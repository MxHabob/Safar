...
import { useListBookingsApiV1AdminBookingsGet } from "@/generated/hooks/admin";
import type {
  ListBookingsApiV1AdminBookingsGetResponse,
...
export function useBookings({ initialBookingsData }: UseBookingsProps = {}) {
  ...
  // Fetch bookings from API with filters - API handles filtering and pagination
  const { data, isLoading, error } = useListBookingsApiV1AdminBookingsGet(
    pagination.pageIndex * pagination.pageSize, // skip
    pagination.pageSize, // limit
    filters.query.status, // status
    {
      enabled: true,
      initialData: shouldUseInitialData ? initialBookingsData : undefined,
    }
  );
  ...
  const sortedBookings = useMemo(() => {
    if (!bookings || bookings.length === 0 || sorting.length === 0)
      return bookings;
    ...
    return [...bookings].sort((a, b) => {
      for (const sort of sorting) {
        const key = sort.id as keyof AdminBookingResponse;
